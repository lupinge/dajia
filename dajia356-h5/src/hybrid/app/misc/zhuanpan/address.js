...
exports.init = function (config) {
    ...
    if (config.user_info) {
        ...
        Service.post('get_lottery_data', null, function (err, data) {
            if (err) {
                Util.toast(err.message);
                return;
            }
            $body.removeClass('loading');
            $body.append(template({user_info:data}));
            Widget.initWidgets();
        });
    }
};
        ...
        'submit [data-role="form"]': function (e) {
            ...
            User.getUserInfo(function (err, userInfo) {
                ...
                Service.post('add_bill_log',
                    ...
                    Dialog.alert('信息提交成功O(∩_∩)O,奖品将于活动结束后统一发放～')
                            .then(function () {
                                if (window.navigator.userAgent.indexOf('Android') > -1) {
                                    NativeAPI.invoke('closeWebview');
                                    return;
                                }
                                window.location.href = '/index.html?v=100#misc/zhuanpan/';
                            });
                });
            });
        },
